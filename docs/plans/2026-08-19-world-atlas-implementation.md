...
全 8 タスク完了。各タスクは実装 → 仕様適合レビュー → 品質レビュー(+修正)を
通過し、最終総合レビューで **READY for P6** 判定。
テスト: ユニット 323 / 23 スイート、UI 38(うち世界クイズの通し 1 本)全緑。
日本版の回帰ゼロ。コミット列(feature/worldmap):
4357809 → da37be8 → 63a6e68 → a2f3ce6 → 6cf4ce8 → 0a29b2b → 92bfdad →
e612e86 → 08ca2a6 → c1cc27a → 960c15f → 56b6884 → e0b525a → 07b8435 →
c55c706 → 0399f21 → fe6bc6f → 5d23d09

## P6 への引き継ぎメモ(コードにも計画書にも無かった知見)

1. **ResultView は日本の本に固定**(ResultView.swift 冒頭の ⚠️ 注釈参照)。
   世界の結果表示には Atlas を通す作業が必須
2. **カード ID が第 3 の衝突軸。** 日本 "12-1"〜"47-1" と ISO 2 桁の世界
   "12-1" 等、**7 件が文字列衝突**する。アトラス横断のカード表示は
   (atlas, id) で引くこと
3. **eraseAll は両方の本を消す**(マイマップの全削除ボタン)。本ごとに
   するかは世界 UI 実装時の製品判断
4. **音声語彙は AppState.init で日本のみ設定**。本の切替時に
   `VoiceInputService.configure` を呼び直す箇所が要る(タイトルのページ
   めくりが自然)
5. **`app.world` は main actor 上の同期 lazy ロード**(日本の約 2 倍)。
   タイトル 2 ページ目が見えたタイミングでの先読みを検討
6. **世界の UI テストは findOnMap 1 問のみ**。結果画面まで通す拡張を
   P6 で(1 の罠が緑のまま隠れた原因)
7. コード内に書かれた罠の再読を: RootView `.stageSelect`(日本ピッカー×
   世界 atlasKey)、Atlas.swift の `isNationwide`(47 前提)、未配線の
   `background` / `insets` / `europeBbox`
8. 履歴由来の tracked `tools/__pycache__/build_map_data.cpython-311.pyc` が
   残っている(9c840f7 から)。どこかで `git rm --cached` を
