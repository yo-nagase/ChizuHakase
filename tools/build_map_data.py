...
from map_geometry import (bbox_gap, bbox_of, point_in_rings,
                          pole_of_inaccessibility, ring_area)
...
# Geometry helpers (ring_area, polylabel, ...) live in map_geometry.py,
# shared with the world pipeline.
...
def main():
    ...
    for code in range(1, 48):
        ...
        # The area centroid is not usable as a label anchor here: it falls in
        # the sea for curved prefectures like Kyoto and outside the landmass
        # entirely for multi-island ones like Nagasaki. CLAUDE.md sec.10
        # requires contains(centroid) to hold for all 47, so it has to be the
        # guaranteed interior point, not an average.
