...
**同じ札をもう 1 枚引くたびに星が 1 つ増える**(上限 15)。

```
未所持のカードがあれば、その中からランダムに 1 枚 (★1)
そうでなければ、★15 未満の札からランダムに 1 枚 → ★ +1
全部 ★15 なら「もっている カードだね」(増えない)
```
...
| **シルバーカード** | ★5–14 |
| **ゴールドカード** | ★15 |
| **にじいろカード** | 金になった後の県のれんぞく正解 7(下記) |

- 未所持を優先することでコレクションが早く埋まり、
  埋まった後は星を伸ばす動機が残る
- **★15 に達していない札から引く。** 全部から引くと、埋まりかけの県では
  当たりが ★15 の札に吸われ、正解しても何も起きない回が増える
- **カード絵はシルバー以上で出す**(§4)。ゴールド(同じ札 15 枚ぶん)まで
  隠すと大半の絵が誰にも見られない
- 星は「持っている枚数」であって強さではない。**減らさない**
- カード面の星は「★12」のように**星 1 個+数字**で描く。15 個は並べられない
...
- **金(★15)が先、れんぞくが後。** 札が金になった瞬間にもカウンタは 0 に戻る
  (金を完成させた答えは数えない)。そこかられんぞくが **7** に達した時点で、
  **その時点で金だった札**が虹になる。金より前に積んだれんぞくは持ち越さない —
  虹は「完成した札を持ったまま走り切った」勲章だから。当初は順不同のれんぞく 15
  だったが、金と同時に虹まで付いてくる回があり、二つの到達が別の出来事にならなかった
- 判定はれんぞくが 7 に**触れた瞬間**に行う(`GameRules.nextStreak` が出題結果と
...
- 虹になった回は**結果画面の先頭で「🌈 にじいろに なった カード!」として祝う**。
  ...
  文字だけの札では何も渡していない
- **この祝いは獲得カード欄では代用できない。** れんぞく 7 に届いた時点で
...
struct SaveData: Codable {
    ...
    var cards: [String: Int] = [:]       // カードID → 星の数 (0...15)
...
- `version` を見て将来のマイグレーションを分岐する。
  ...
  **v3 → v4**(星の尺度を 15 段階に)、**v4 → v5**(習熟を 5 回制に)は
  `SaveData.init(from:)` の中で行う。
  v3 では **旧「キラ」(所持数 2)を旧スケールの上限に読み替える**(§12:
  すでに手に入れたキラを取り上げない)。
  v4 では旧★を**段位の位置で**写す: 1→1, 2→2, 3→**5**(銀の下限),
  4→**10**(銀区間の中間), 5→**15**(金のまま)。子どもが見ているのは段位
  であり、段位を落とさないことが基準。**虹は付与しない** — れんぞくの勲章で、
  まだ誰も歩いていない。
...
- **カードの絵柄は 1 つ(`CardFaceView`)。** ずかん・マイマップ・結果画面の
  ...
  一覧からは分からなかった
- ★1–4 は寒色のマット台紙、★5 は銀の箔、★15 は金の箔、虹はパステルの虹箔。
