...
- カード詳細（`CardDetailView`）は**トレカのテンプレートを避ける**。
  色枠・タイトル帯・額装・名前プレート・★レア・番号フッターを積み上げた形は
  「カードとは何か」の一般解であって、どのカードでもない。
  代わりに**掛け紙・引き札**の語彙を使う: 縦組みの県名帯、端まで届く絵、
  朱印、二重罫。**絵の中に既にある情報は枠側で繰り返さない**
  （絵は漢字で「宮城県 ずんだ餅」と刷ってあるので、枠側は**読み**を担う）
